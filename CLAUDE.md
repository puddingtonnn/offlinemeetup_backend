...
- **`internal/dto/`** — request/response shapes. A **layer-agnostic shared kernel**: transport, service and cache all depend on it. It deliberately lives at `internal/dto` (not under `transport/http`) so the import graph stays honest — a leaf type every layer may use, not one the HTTP layer "owns" that the SQL/cache layers reach up into. The **repo never imports `dto`**: `MeetupRepo.List` takes `repo.MeetupQuery` (a plain criteria struct, no json tags) and the service maps `dto.MeetupFilter → repo.MeetupQuery` at its boundary.
...
- **`internal/transport/http/`** — `router.go` (all routes + middleware groups), `handler/` (HTTP handlers + Swagger annotations; `handler/helpers.go` holds `requireUserID`/`pathInt64` — use them for the auth-guard + URL-int parse so a bad id maps to 400 via `ErrInvalidInput`, never a bare `fmt.Errorf` that falls through to 500), `middleware/`, `response/`.
...
`response.RespondError` is the single mapping point; it logs **by status class** — a 5xx at `slog.Error` (a server fault worth alerting), a 4xx at `slog.Info` (expected client control-flow: bad input, 404, expired token). Don't log every failure at Error, and don't hand `RespondError` a bare `fmt.Errorf` for a client mistake — it isn't a sentinel, so it maps to 500. Use the `service.Err*` sentinels (or the `pathInt64`/`requireUserID` helpers, which already do).
...
- **Mutation authz reads scalars, not the whole graph.** `JoinMeetup`/`LeaveMeetup`/`DeleteMeetup` call `repo.GetForAuth(ctx, id, userID)` — a projection of `creator_id`/`is_public`/`status`/`end_time` + an `is_member` EXISTS — instead of `GetByID`, which hydrates the full creator/participant/tag graph (a whole participant roster) just to check a few columns. Keep `GetByID` only where the full DTO is returned (`GetMeetup` cache loader, `UpdateMeetup` which mutates the row). `meetups.creator_id` is indexed (`idx_meetups_creator_id`, created `CONCURRENTLY`) for this path and the "my created" list.
...
**Idempotent send (`request_id`).** `SendMessage` takes an optional client-supplied `request_id` (idempotency key, carried in the REST body and on the WS envelope) so a retry from the mobile upload queue doesn't create a duplicate. Scope is **`(chat_id, sender_id, request_id)`** — a partial unique index (`WHERE request_id IS NOT NULL`), so the same key reused in a *different* chat creates a separate message rather than silently dropping it. De-dup is **atomic in the repo**, not check-then-act: `SaveMessage` first `SELECT`s an already-committed message for the key (returns it with `created=false`, **skipping** the read-only/membership guards since they passed at creation — a retry after the chat went read-only still returns the original), then falls through to `INSERT ... ON CONFLICT (chat_id, sender_id, request_id) DO NOTHING` to catch a concurrent new insert; `created = RowsAffected > 0`. The service normalizes `""→nil` and rejects `request_id` longer than 128 chars (it's in a btree index). REST returns **201 + broadcast** on create, **200 + no broadcast** on a duplicate; WS broadcasts to all on create, to the sender only on a duplicate. `SaveMessage` returns `(msg, targetIDs, created, error)`; `request_id` is echoed in the message DTO and history.
...
**Connection teardown / channel ownership.** `client.send` has **multiple senders** (hub broadcasts via `trySend`, `sendError`, the presence snapshot in `announcePresence`), so it is **never closed** — closing a channel that other goroutines still send to panics (`send on closed channel`). Teardown goes through `client.stop()`, which cancels the connection ctx: `writePump` exits on `ctx.Done` and closes the socket, which in turn unblocks `readPump`. The hub's `unregister` case and shutdown (`ctx.Done`) call `client.stop()`, **not** `close(client.send)`. Do not reintroduce `close(client.send)` — that was a real panic vector. `readPump`'s teardown unregisters via `unregisterFromHub`, which **selects on `hub.done`** (closed by `Run` right before it returns on shutdown) so a graceful restart never blocks forever on a channel whose reader (`Run`) already exited — cleanup (`markOffline`, last_seen) still runs.

**Goroutine safety.** Every goroutine the WS layer spawns runs through `safeGo`/`recoverPanic` (`safego.go`). A panic at the top of a bare goroutine calls `os.Exit(2)` and takes the whole instance down (chi's `middleware.Recoverer` only wraps the request goroutine, not ones you spawn), so read/write pumps, the event worker and heartbeats all recover into a logged error scoped to one connection. **Inbound events are not `go`-per-frame.** `readPump` is `work`'s only sender and enqueues decoded events (non-blocking; buffer full ⇒ `sendError("too fast")` backpressure) to a **single per-connection worker** `eventPump`, which processes them **in order** (so two messages A→B persist A then B) and recovers **per event** (one bad message can't kill the worker). This replaced an unbounded `go func` per frame that had no concurrency bound and reordered messages.
