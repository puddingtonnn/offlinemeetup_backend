...
// presenceEvent builds a userOnline / userOffline WS frame. lastSeen is a unix
// timestamp, set only for offline events.
func presenceEvent(eventType string, userID int64, online bool, lastSeen *int64) []byte {
	payload, _ := json.Marshal(WSPresencePayload{UserID: userID, Online: online, LastSeen: lastSeen})
...
func presenceSnapshotEvent(statuses []service.PresenceStatus) []byte {
	...
	for _, st := range statuses {
		...
		users = append(users, WSPresencePayload{UserID: st.UserID, Online: st.Online, LastSeen: ls})
	}
